...
<<dataReductionGroupClickPost, echo=F,eval=T>>=
  ...
  names(tlc)[names(tlc) == "Factor1"] <- "groupClickPost"
...
<<factorsCorrelationMatrixes, echo=F,eval=T>>=

postDrillFactors <- subset(twc, select = c(groupConfidentTechnicalChallenges_2, indConfidenceChallenges_2, groupPerformance_3, indPerformance_3, groupClickFactorPost, groupBondingFactorPost, teamBondingFactorPost))
postDrillFactors <- cor(postDrillFactors, use = "complete")



#tlc$groupPerfChangePrePost <- tlc$groupPerformanceExpectations[tlc$time == 3] - tlc$groupConfidentTechnicalChallenges[tlc$time == 2]
twc$teamBondingFactorPrePost <- (twc$teamBondingFactorPost - twc$teamBondingFactorBaseline)

prePostDrillFactors <- subset(twc, select = c(groupConfidentTechnicalChallenges_2Norm, indConfidenceChallenges_2,
                                                   groupPerformance_3, indPerformance_3,
                                                   groupClickChangePrePost, groupBondingChangePrePost, teamBondingFactorPrePost))
prePostDrillFactorsMatrix <- cor(prePostDrillFactors, use = "complete")
...
library(tidyverse)
  ...
  dev.copy(pdf, "teamPerfExpClickScatter.pdf")
  dev.off()
...
library(texreg)
  texreg(list(groupPerfExpClick.intercept
                                  , groupPerfExpClick.main
                                  , groupPerfExpClick.main.controls)
         , custom.coef.names = c("(constant)", "Group Performance Vs Expectations", "Ind Performance Vs Expectations"
                                  , "Arousal", "Trial Outcome Avg.","Subjective Competence", "Objective Competence","Extraverted")
         , custom.model.names = c("Intercept", "Main effect", "Controls")
         , booktabs = TRUE
         , use.packages = FALSE
         ...
         , scriptsize = TRUE
         , table = TRUE
         , file = "TEM1groupPerfExpClick.tex")
...
library(lmerTest)
  ...
  #calculate marginal (fixed) and conditional (random) R^2 for the model:
  #clickBonding.controls.R2 <-
  rsquared(clickBonding.controls)
...
<<clickBondingCooksD, echo=F,eval=T>>=
  ...
  dev.off()
    ...
    texreg(list(clickBonding.intercept, clickBonding.main, clickBonding.controls)
           , custom.coef.names = c("(constant)", "Team Click", "Arousal", "Trial Outcome Avg.", "Subjective Competence", "Objective Competence", "Extraverted")
           , custom.model.names = c("Intercept", "Main effect", "Controls")
           , booktabs = TRUE
           , use.packages = FALSE
           ...
           , bold = 0.05
           , ci.force = FALSE
           , custom.note = "Coefficients with $p < 0.05$ in \\textbf{bold}. Marginal $R^2 = .59$, Conditional $R^2 = .59$"
           , scriptsize = TRUE
           , table = TRUE
           , file = "TEM2clickBonding.tex")
...
<<clickTeamBondingModel, eval=T,echo=F>>=
  ...
  clickTeamBonding.intercept <- lmer(teamBondingFactorPost ~ 1 + ( 1 |sessionName),
                              data = twc,
                              REML = F)
  clickTeamBonding.main <- lmer(teamBondingFactorPost ~ groupClickPostFactor +
                              ( groupClickPostFactor |sessionName),
                              data = twc,
                              REML = F)
...
rsquared(clickBonding.controls)
 ...
 # interaction is not significant and so dropped from analysis
  ...
  #calculate marginal (fixed) and conditional (random) R^2 for the model:
  #clickTeamBondingCondition.controls.R2 <-
  rsquared(clickTeamBondingCondition.controls)

anova(clickTeamBonding.controls, clickTeamBondingCondition.controls)


texreg(list(clickTeamBonding.intercept
                      ,  clickTeamBonding.controls
                      , clickTeamBonding.controls)
       ...
       , scriptsize = TRUE
       , table = TRUE
       , file = "clickTeamBondingTable.tex"
     )
...
perfClickChange.intercept <- lmer(groupClickChangePrePostNorm ~ 1 + ( 1 |sessionName),
                            data = twc,
                            REML = F)
perfClickChange.main <- lmer(groupClickChangePrePostNorm ~ groupPerfChangePrePostNorm
                            + ( 1 | sessionName),
                            data = twc,
                            REML = F)
#summary(groupPerfExpClick.main)
...
summary(perfClickChangeCondition.lmer <- lmer(groupClickChangePrePostNorm ~ groupPerfChangePrePostNorm
                                    + indPerfChangePrePostNorm
                                    + arousalFactorPost
                                    + fullOutcomeAvg
                                    + subjectiveCompetenceFactor + objectiveCompetenceFactor
...
<<groupPerfClickChangeCooksD, echo=F,eval=T>>=
 ...
 library(influence.ME)
  ...
  texreg(list(perfClickChange.intercept
                                  , perfClickChange.main
                                  , perfClickChangeCondition.lmer)
         , custom.coef.names = c("(constant)"
                                  , "Group Performance Vs Expectations Change"
                                  , "Ind Performance Vs Expectations Change"
                                  , "Arousal"
                                  , "Trial Outcome Avg."
                                  , "Subjective Competence"
                                  , "Objective Competence"
         ...
         , scriptsize = TRUE
         , table = TRUE
         , file = "TEM21groupPerfExpClickChange.tex")
...
#intercept
summary(clickBondingChangeIntercept.lmer <- lmer(groupBondingChangePrePostNorm ~ 1
                                    + (1 | sessionName)
                                    , data = twc
                                    , REML = FALSE
                                    , control = lmerControl(optimizer = "optimx",
...
#main effect:
summary(clickBondingChangeMain.lmer <- lmer(groupBondingChangePrePostNorm ~ groupClickChangePrePostNorm
                                    + (groupClickChangePrePostNorm | sessionName)
                                    , data = twc
                                    , REML = FALSE
                                    , control = lmerControl(optimizer = "optimx",
                                      calc.derivs = FALSE,  optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
...
#install.packages("piecewiseSEM")
library(piecewiseSEM)
rsquared(clickBondingChange.lmer)

@

<<groupClickBondingChangeModelTable, eval=T, echo=F>>=
library(texreg)

texreg(list(clickBondingChangeIntercept.lmer, clickBondingChangeMain.lmer, clickBondingChange.lmer)
       , custom.coef.names = c("(constant)", "Team Click Change", "Ind Performance Change", "Trial Outcome Avg.", "Subjective Competence", "Objective Competence",
                               "Extraverted")
       , custom.model.names = c("Intercept", "Main effect", "Controls")
       #, dcolumn = TRUE
       , booktabs = TRUE
       , use.packages = FALSE
       , label = "tab:groupClickBondingChange"
       , caption = "Prediction 3.a (Post-Drill): Team Click Change predicts Social Bonding Change (n = 53)."
       , single.row = FALSE
       , bold = 0.05
       , ci.force = FALSE
       , custom.note = "Coefficients with $p < 0.05$ in \\textbf{bold}. Marginal $R^2 = .23$, Conditional $R^2 = .24$"
       , scriptsize = TRUE
       , table = TRUE
       , file = "groupClickBondingChange.tex")
@
%Marginal $R^2 = .56$, Conditional $R^2 = .61$"
