...
Current research suggests that, faced with these minimal requirements for joint action, humans have devised a number of solutions. Faculties for joint action involve re-purposing of mechanisms core to successful intra-personal coordination, such as sensory motor simulation \citep{Vesper2012}, direct coupling of action and perception networks \citep{Novembre2014}, and lower cognitive mechanisms of movement regulation \citep{Semin2008}.  Generally speaking, these solutions appear to exist on a continuum, with more (neuro)computationally intensive mechanisms of anticipation, attention, and adaption occupying one end, and extra-neural mechanisms of more direct coupling with other components of the movement system occupying the other.  Successful joint action relies on the recruitment of a suite of mechanisms from this continuum in order to establish and sustain interpersonal coordination.  Evidence (discussed below) suggests that optimal solutions to joint action may tend recruit more extra-neural resources to minimise free energy, whereas less efficient solutions to joint action may rely on computationally more intensive procedures in order to reduce uncertainty.
...
Compare CONTROL THEORY to Predictive coding HERE
...
\myparagraph{PJAM}
Pesquita and colleagues \textcite{Pesquita2017} propose that the best model for accounting for the minimal requirements of joint action is through a hierarchical predictive approach, which they term a ``Predictive Joint Action Model'' (PJAM).
...
The model is composed of three hierarchical levels of inference: goal representation, action-planning, and sensory routing (see figure ~\ref{fig:PJAM}). PJAM assumes that each participant in a joint-action maintains internal models of 1) the joint task, 2) the action contributions of self and others to the joint task, and  3) the sensory-motor inputs expected from joint activity.
