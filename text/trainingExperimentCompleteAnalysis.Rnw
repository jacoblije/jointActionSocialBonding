...
\begin{\begin{description}
  ...
  \item[Prediction 1.a:] More positive violations of expectations around team performance will correlate with higer feelings of team click with the training group
...
Athletes completed survey questionnaires via WeChat using their personal mobile devices and internet connection. Surveys were administered at three time points: 1) one day before the experiment (Baseline), 2) immediately before the experiment after receiving the experimental prime (Pre-Experiment), and 3) once immediately following the completion of the experiment (Post-Experiment)(see ~\ref{fig:surveyMeasureSummaryTable}).
...
The KMO index and Bartlett's test both suggested high sampling adequacy, (KMO =  \Sexpr{KMOteamDisciplineMatrix$MSA}, \chi^2(\Sexpr{bartlettTeamDiscipline$df}, N = \Sexpr{teamDisciplineSampleSize}) = \Sexpr{bartlettTeamDiscipline$chisq}, p = \Sexpr{bartlettTeamDiscipline$p.value}).
One factor, labelled ``Team Discipline'' was imposed on the data, which explained \Sexpr{prVar.teamDisciplineFA} \% of the overall variance (SS Loading = \Sexpr{SSLoadings.teamDisciplineFA}).  Guttmans \lambda = \Sexpr{reliabilityTeamDiscipline$total[,3]}  and  Cronbachs \alpha = \Sexpr{reliabilityTeamDiscipline$total[,2]}  indicated that the data reduction was appropriate and reliable.
...
<<groupPerfExpClickModel, echo=F,eval=T>>=
 ...
 groupPerfExpClick_lmer_results <- papaja::apa_print(groupPerfExpClick.controls)

 class(groupPerfExpClick.controls)
unclass(groupPerfExpClick.controls)
 recall_anova_results_p <-apa_print(recall_anova, es = "pes", in_paren = TRUE

@

Figure ~\ref{fig:teamPerfExpClickScatter} shows a strong positive correlation between violations of prior expectations concerning team performance and feelings of ``click'' with the training group post-Experiment.  The slope of this relationship appears to vary according to condition, as indicated on the scatterplot by the two non-parallel lines of best fit.  A LMER was used to model this relationship: Group Performance Expectations, Condition, and their interaction were included as fixed effects, and experiment session was modelled as a random effect (intercept and slope).  Perceptions of individual performance relative to prior expectations, post-Experiment arousal, the average performance outcome of each invasion drill trial, and subjective and objective measures of athlete technical competence were included as fixed effects (controls).

The model revealed a signigicant positive main effect of group performance expectation violation on feelings of ``team click'' with the training group, \Sexpr{coef(summary(groupPerfExpClick.controls))["groupPerformance_3Norm","Estimate"]}

$\beta = .27$ ($95\% CI =  .05, .49$), $SE = 0.11$, $t(32.97) = 2.36$, $p = .02$, $marginal R^2 = .12$, $conditional R^2 = .22$
.  The main effect of condition, \Sexpr{}, and the interaction effect of group performance expectation violation and condition on click was also significant, \Sexpr{}.  In addition, average performance outcome of each invasion drill trial was also a significant positive predictor of team click, \Sexpr.  These results suggested that group performance expectation violations were a positive predictor of feelings of group click among athletes, and that this relationship was moderated by experiment condition, such that expectation violation had a greater effect on feelings of group click in the high difficulty condition than the low difficulty condition.  Model residuals were normally distributed around zero, ($W = 0.98, p = .77$), and an analysis of Cook's distances suggests that the model did not contain any overly influential observations (all < .8) ~\ref{fig:M1Assumptions}), suggesting that the model provided a robust basis for inference. In line with predictions, more positive violation of expectations concerning group performance predicted feelings of team click, and this result was, as predicted, moderated by condition such that the effect was stronger in the high difficulty condition.
...
<<groupPerfExpClickAssumptionsScatter, echo=F,eval=T>>=
  ...
  dev.off()
@
<<groupPerfExpClickAssumptionsQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  ...
  dev.off()
@
<<groupPerfExpClickAssumptionsCooksD, echo=F,eval=T>>=
  #indluential cases
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
  plot(cooks.distance(groupPerfExpClick.controls), main = "M1 Influential cases \n (Cook's Distance)", ylab = "cooks.distance")
...
Figure ~\ref{fig:jASGroupPerfExpClickScatterLMER} displays the relationship between perceptions of Joint Action Success and Group Click, moderated by violation of group performance expectations. The dotted and dashed lines either side of the solid line represent one standard deviation below and above the mean of Group Performance Expectations.  A LMER confirms negative interation effect of Group Performance Expectations and Joint Action Success on Group Click, such that the relationship between joint action success an social bonding is more pronounced for less positive violations of expectations around group performance.

Model residuals were normally distributed around zero, W = 0.99001, p-value = 0.9192
($W = 0.98, p = .77$), and an analysis of Cook's distances suggests that the model did contain one influential observation (Cook's Distance > < 1.0) ~\ref{fig:M1Assumptions}).  Re-running the model with this value excluded led to an improvement of the model fit,

suggesting that the model provided a robust basis for inference. In line with predictions, more positive violation of expectations concerning group performance predicted feelings of team click, and this result was, as predicted, moderated by condition such that the effect was stronger in the high difficulty condition.
...
<<jASgroupPerfExpClickIntHist, echo=F,eval=T>>=
  ...
  dev.off()
  #non-normality
  shapiro.test(jASgroupPerfExpClickIntResid)
...
Figure ~\ref{fig:groupClickBondScatter} shows a clear relationship between Group Click and Social Bonding to the training group following the training group.  A linear mixed effects regression, controlling for arousal, group performance outcome, and technical competence (subjective and objective), revealed a strong positive relationship between feelings of group click and feelings of social bonding to the training group.

Residuals:
W = 0.98766, p-value = 0.8568
...
<<clickBondingHist, echo=F,eval=T>>=
  ...
  hist(clickBondingResid, freq = FALSE, main = "Distribution of M2 Residuals", xlab = "Residuals")
...
<<clickBondingScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(clickBonding.controls, main = "Scatterplot of M2 Residuals")
  dev.copy(pdf, "TEM2Scatter.pdf")
  dev.off()
@
<<clickBondingQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(clickBonding.controls), main = "Norm QQ Plot of M2 Residuals")
...
<<clickBondingCooksD, echo=F,eval=T>>=
  #indluential cases
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
  plot(cooks.distance(clickBonding.controls), main = "M2 Influential Cases \n (Cook's Distance)", ylab = "cooks.distance")
...
<<groupPerfExpBondScatterCondition, eval=T,echo=F, fig.cap= 'Positive violation of group performance expectations \n predict feelings of Social Bonding to training group'>>=
...
<<groupPerfExpBondingConditionHist, echo=F,eval=T>>=
  ...
  dev.off()
  #non-normality
  shapiro.test(groupPerfExpBondingConditionResid)
...
Results of the mediation analysis revealed significant average indirect effect of Group Performance Expectation Violation on Social Bonding attributable to Team Click, \Sexpr{}RESULTS.  When controlling for the effect of team click on social bonding, the average direct effect between Joint Action Success and Social Bonding was no longer significant, \Sexpr{} (see Figure ~\ref{fig:groupPerfExpClickMediationPlot}). The direct effect diminished such that including Joint Action Success in the model produced a total effect that was marginally \textit{smaller} than the indirect effect alone, $\beta = .36, 95\% CI = .13 , .61, p = .01$. These results suggest that feelings of team click fully mediate the relationship between perceptions of joint-action success and social bonding.
