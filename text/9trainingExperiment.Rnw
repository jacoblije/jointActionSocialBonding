...
<<groupConfidenceConditionLMER, echo=F,eval=T>>=
  ...
  # model
  summary(conditionGroupConf.main <- lmer(groupConfidentTechnicalChallenges_2 ~ condition
...
summary(indConfChallenges.main <- lmer(indConfidenceChallenges_2 ~ condition
                            + objectiveCompetenceFactor + subjectiveCompetenceFactor 
                            + (1|sessionName)
                            , data = twc
                            , REML = F
...
<<groupPerfExpClickModel, echo=F,eval=T>>=
  ...
  library(lmerTest)
  summary(groupPerfExpClick.controls <- lmer(groupClickPostFactor ~
...
,
  \betavec
    \Sexpr{coef(summary(groupPerfExpClick.controls))["fullOutcomeAvg","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpClick.controls.CIs["fullOutcomeAvg", 1]}
  ,
    \Sexpr{groupPerfExpClick.controls.CIs["fullOutcomeAvg", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpClick.controls))["fullOutcomeAvg","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(groupPerfExpClick.controls))["fullOutcomeAvg","df"]}
  )
  =
    \Sexpr{coef(summary(groupPerfExpClick.controls))["fullOutcomeAvg","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(groupPerfExpClick.controls))["fullOutcomeAvg","Pr(>|t|)"]}
...
<<clickBondingModel, eval=T,echo=F>>=
  ...
  summary(clickBondingCondition.controls <- lmer(groupBondingFactorPost ~
...
library(optimx)
  ...
  summary(groupPerfExpBondingCondition.main)
    ...
    summary(groupPerfExpBondingCondition.controls <- lmer(groupBondingFactorPost ~
                                            groupPerformance_3Norm*condition
                                            + indPerformance_3Norm + arousalFactorPost + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                            + (groupPerformance_3Norm |sessionName)
...
#  library(lmerTest)
  ...
  summary(m1 <- lmer(groupBondingFactorPost ~ groupPerformance_3Norm*condition
  ...
  summary(m2 <- lmer(groupClickPostFactor ~ groupPerformance_3Norm*condition
                                            + indPerformance_3Norm + arousalFactorPost + fullOutcomeAvg
                                            + subjectiveCompetenceFactor + objectiveCompetenceFactor
  ...
  summary(m3 <- lmer(groupBondingFactorPost ~  groupPerformance_3Norm + groupClickPostFactor*condition + groupPerformance_3Norm*condition
                                      + arousalFactorPost + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
...
twc$indPerfExpChangePrePostNorm <- scale(twc$indPerfChangePrePost)
...
groupPerfClickChange <- ggplot(twc,
                            aes(x=groupPerfChangePrePostNorm, y=groupClickChangePrePostNorm, color=condition)) +
                            geom_point(shape = 1) +
                            geom_smooth(method=lm, se=FALSE)   # Don't add shaded confidence region
groupPerfClickChange +
            xlab("Group Performance Change") +
            ylab("Group Click Change") +
            apatheme +
...
summary(perfClickChangeCondition.lmer <- lmer(groupClickChangePrePostNorm ~ groupPerfChangePrePostNorm*condition
                                    + indPerfExpChangePrePostNorm + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
...
<<groupPerfClickChangeAssumptionsHist, echo=F,eval=T>>=
  #1.historgram with density line
  groupPerfClickChangeResid <- resid(perfClickChangeCondition.lmer, type = "pearson")
  hist(groupPerfClickChangeResid,
       freq = FALSE,
       main = "Distribution of M2.1 residuals \n (standardised)",
       xlab = "Residuals")
...
<<groupPerfClickChangeAssumptionsScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(perfClickChangeCondition.lmer, main = "Scatterplot of M2.1 Residuals \n (standardised)")
  dev.copy(pdf, "TEM21Scatter.pdf")
  dev.off()
@
<<groupPerfClickChangeQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(perfClickChangeCondition.lmer), main = "Norm QQ Plot of M2.1 Residuals")
  qqline(residuals(perfClickChangeCondition.lmer))
...
<<groupPerfClickChangeCooksD, echo=F,eval=T>>=
  ...
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
 library(influence.ME)
  groupPerfChangeClickInfl <- influence(perfClickChangeCondition.lmer, obs = TRUE)
...
groupClickBondingChange <- ggplot(twc,
                            aes(x=groupClickChangePrePostNorm, y=groupBondingChangePrePostNorm, color=condition)) +
                            geom_point(shape = 1) +
                            geom_smooth(method=lm, se=FALSE)   # Don't add shaded confidence region
groupClickBondingChange +
            xlab("Group Click Change") +
...
<<clickBondChangeAssumptionsHist, echo=F,eval=T>>=
  #1.historgram with density line
  clickBondChangeResid <- resid(clickBondingChangeCondition.lmer, type = "pearson")
  hist(clickBondChangeResid,
       freq = FALSE,
       main = "Distribution of M2.2 residuals \n (standardised)",
       xlab = "Residuals")
...
<<clickBondChangeAssumptionsScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(clickBondingChangeCondition.lmer, main = "Scatterplot of M2.2 Residuals \n (standardised)")
  dev.copy(pdf, "TEM22Scatter.pdf")
  dev.off()
@
<<clickBondChangeQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(clickBondingChangeCondition.lmer), main = "Norm QQ Plot of M2.2 Residuals")
  qqline(residuals(clickBondingChangeCondition.lmer))
...
<<clickBondChangeCooksD, echo=F,eval=T>>=
  ...
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
 library(influence.ME)
  clickBondingChangeInfl <- influence(clickBondingChangeCondition.lmer, obs = TRUE)
...
groupPerfBondingChange <- ggplot(twc,
                            aes(x=groupPerfChangePrePostNorm, y=groupBondingChangePrePostNorm, color=condition)) +
                            geom_point(shape = 1) +
                            geom_smooth(method=lm, se=FALSE)   # Don't add shaded confidence region
...
<<perfBondChangeAssumptionsHist, echo=F,eval=T>>=
  #1.historgram with density line
  perfBondChangeResid <- resid(perfBondingChangeCondition.lmer, type = "pearson")
  hist(perfBondChangeResid,
       freq = FALSE,
       main = "Distribution of M2.3 residuals \n (standardised)",
       xlab = "Residuals")
...
<<perfBondChangeAssumptionsScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(perfBondingChangeCondition.lmer, main = "Scatterplot of M2.3 Residuals \n (standardised)")
  dev.copy(pdf, "TEM23Scatter.pdf")
  dev.off()
@
<<perfBondChangeQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(perfBondingChangeCondition.lmer), main = "Norm QQ Plot of M2.3 Residuals")
  qqline(residuals(perfBondingChangeCondition.lmer))
...
<<perfBondChangeCooksD, echo=F,eval=T>>=
  ...
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
 library(influence.ME)
  perfBondingChangeInfl <- influence(perfBondingChangeCondition.lmer, obs = TRUE)
  ...
  detach_package <- function(pkg, character.only = FALSE)
{
 ...
 #library(lmerTest)
  ## Direct relationship X->Y: fit model and show summary
  summary(m1Change <- lmer(groupBondingChangePrePost ~ groupPerfChangePrePostNorm*condition
                                              + indPerfExpChangePrePostNorm + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                              + (groupPerfChangePrePostNorm|sessionName)
                                              , control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,
                                                                    optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
  ...
  ## X->M fit model and show summary
  summary(m2Change <- lmer(groupClickChangePrePost ~ groupPerfChangePrePostNorm*condition
                                            + indPerfExpChangePrePostNorm + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                            + (groupPerfChangePrePostNorm|sessionName)
                                            , control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,
                                                                  optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
  ...
  summary(m3Change <- lmer(groupBondingChangePrePost ~  groupClickChangePrePost
                                          + groupPerfChangePrePostNorm*condition + groupClickChangePrePost*condition
                                          + indPerfExpChangePrePostNorm + fullOutcomeAvg + subjectiveCompetenceFactor + objectiveCompetenceFactor
...
plot(med.out.groupPerfClickChange, main = "Mediation Model")
...
<<>>=
plot(med.out.groupPerfClickChange.High, main = "Mediation Model: High")
dev.copy(pdf, "groupPerfExpClickChangeMedPlotHigh.pdf")
dev.off()
@

<<groupPerfExpClickChangeMedPlotLow, eval=T,echo=F>>=
plot(med.out.groupPerfClickChange.Low, main = "Mediation Model: Low")
