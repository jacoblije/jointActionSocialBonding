...
(
men =
  \Sexpr{length(which(twc$sex == 0))}
M(age) =
  \Sexpr{mean(twc$age, na.rm = TRUE)}
(
SD =
  \Sexpr{sd(twc$age, na.rm = T)},
range = 16-29
)
).
Athletes were recruited from two provincial rugby programs,
  \Sexpr{length(which(twc$team == "sdw" | twc$team == "sdm"))}
participants were athletes from Shandong province
(
men =
  \Sexpr{length(which(twc$team == "sdm"))},
M(age) =
  \Sexpr{mean(twc$age[twc$team == "sdm" | twc$team == "sdw"], na.rm = T)}
)
and the remaining
  \Sexpr{length(which(twc$team == "bjm" | twc$team == "bjw"))}
athletes were from Beijing province
(
men =
  \Sexpr{length(which(twc$team == "bjm"))},
M(age) =
  \Sexpr{mean(twc$age[twc$team == "bjm" | twc$team == "bjw"], na.rm = T)}
).
...
Two different pre-experiment introductions were developed for the same training drill in order to manipulate athletes’ expectations around the technical difficulty of the impending joint-action task.  Athletes in the ``high difficulty'' condition learned that the overall average difficulty rating for the training drill provided by World Rugby coaches and athletes was 77.5/100, or approximately 8 out of 10.  Athletes were told that the drill would require an extension of their abilities as individuals and as a training group (see appendix for full script).  Athletes in the ``low difficulty'' condition, by contrast, learned that they would be participating in an ostensibly different drill with an overall average difficulty rating (provided by World Rugby coaches and athletes) of 22.5/100, or approximately 2 out of 10.  In reality, the training drill was exactly the same for each experimental session, and the relative difficulty of this drill was estimated in a pilot study to be approximately 5/10, see APPENDIX).  By either over- or under-rating the difficulty of the impending training drill, it was hoped that athletes' expectations around certainty of performance would be altered, such that athletes in the high difficulty condition would experience on average higher positive violation of expectations around team performance, whereas athletes in the low difficulty condition would experience less positive violations of expectations around team performance. Surveys were designed and administered using Qualtrics software (Qualtrics version 9, Provo, UT). Survey data were processed and analysed using the R environment (R Development Core Team, 2006).


\subsubsection{Measures}
Athletes completed survey questionnaires via WeChat using their personal mobile devices and internet connection. Surveys were administered at three time points: 1) one day before the experiment (Baseline), 2) immediately before the experiment after receiving the experimental prime (Pre-Experiment), and 3) once immediately following the completion of the experiment (Post-Experiment)(see ~\ref{tab:surveyItemsByTime}).

At Baseline, athletes were asked about their impressions of recent individual performance and the performance of their province team as a whole.  Athletes also responded to questions about feelings of team click and social bonding with their team as a whole. In addition, the Baseline survey also included items concerning basic personal information, current injury status, subjective and objective measures of tehcnical competence, and personality type.  At time 2, Pre-Experiment, athletes were asked questions about their current feelings and expectations concerning their individual performance and the performance of their specific training group in the impending training drill. For example,  athletes were asked ``How do you feel right now about your individual performance (100 point scale, 0 = ``Extremely bad'', 100 = ``Extremely good'').  Athletes were also asked about feelings of team click and social bonding to their specific training group, for example: ``How emotionally supportive does your training group feel right now?'' (100 point scale, 0 = ``Extremely weak'', 100 = ``Extremely strong''. For a full explanation of Pre-Experiment survey items, see Appendix ~\ref{}).  At time 3, Post-Experiment, athletes completed questions concerning their impression of their own performance and the performance of their specific training group in the training drill, relative to their prior expectations. Athletes were also asked about their feelings of team click and social bonding to the training group as well as the provincial team as a whole (team-focussed questons mirrored the survey items that were asked one day earlier at Baseline).
...
Approximately 24 hours before the experiment session, athletes were instructed to complete the baseline survey by opening a link provided in the WeChat group.  This survey included written consent for the study. Approximately 1 hour before the experiment was due to take place, athletes were administered with an experimental prime via WeChat.  In the ``high difficulty'' condition, athletes were primed to believe that they were about to participate in a very difficult training drill (on the upper end of their individual and group technical abilities), whereas athletes in the ``low difficulty'' condition were primed to believe that they were about to participate in an easy training drill well within their tehcnical abilities (see APPENDIX for full script). All athletes were lead to believe that athletes in the other experiemntal groups were performing different drills to their assigned drill. In reality, the training drill for each condition was identical, the only thing that varied was the pre-experiment prime.

Once athletes were assembled at the designated training field, the difficulty prime that had been sent via WeChat an hour earlier was verbally re-administered to the athletes. In addition, athletes were told in more detail about the requirements of the Invasion Drill (see APPENDIX for full details).  Athletes were told that assessment of performance in the experiment would be based on video analysis of an individual's performance in attack and defense, including an athlete's ability to coordinate attack and defense with others. Athletes were provided with no other explicit information regarding performance goals, besides completing the experimental drill to the best of their ability within the rules of rugby union.

Athletes were then administered with the time 2 survey (pre-experiment, post-manipulation), which took approximately 3-5 minutes to complete.  After every athlete had finished the survey, athletes participated in a ~10 minute standard warm up routine, including slow jogging and dynamic stretching.  Athletes were instructed not to use the rugby ball during this period.  Once athletes had completed the warm up, the researcher assembled the group within the training drill area, with one athlete randomly assigned to stand at one of 8 available plastic markers (6 in the case of the modified drills).  In this position, athletes were told once more about the structure and procedure for the training drill, in particular the way in which athletes were expected to rotate clockwise after every trial of the drill so that athletes did not habituate to certain positions or alliances in the drill.
...
<<dfAthleteSummary, echo=F,eval=T>>=
  ...
  dfAthleteSummaryOverall <- dfAthleteSummaryOverall %>%
                                mutate(condition = recode(condition, "high" = "Overall"),
                                       condition = recode(condition, "low" = "Overall")
                                       )
  ...
  dfAthleteSummary <- rename(dfAthleteSummary, Position = startingReserve,
...
((\Sexpr{round(length(which(twc$teamStatus == 2)/length(unique(twc$englishName)))*100, digits = 0)}\%))), or on a short-term trial basis ((\Sexpr{round(length(which(twc$teamStatus == 1)/length(unique(twc$englishName)))*100, digits = 0)}\%))).
Only \Sexpr{length(which(twc$startingReserve == "Starting Team"))} of \Sexpr{length(unique(twc$englishName))} (\Sexpr{round(length(which(twc$startingReserve == "Starting Team"))/length(unique(twc$englishName))*100, digits = 0)}\%) of athletes declared that there were in the starting team of their respective provincial teams.
...
<<descriptivesGroupPerformance, echo=F,eval=T>>=
  ...
  ## create a variable list which we want in table one
  listVars <- c("GroupPerformVsExpected", "ConfidenceInGroupAbility", "GroupAttack", "GroupDefence", "GroupCommunication", "GroupSupportPlay")
...
<<descriptivesIndividualPerformance, echo=F,eval=T>>=
  ...
  ## create a variable list which we want in table one
  listVars <- c("PerformanceVsExpectations", "ConfidenceInAbility", "Defence", "Passing", "SupportPlay", "DecisionMaking", "OverallPerformance")
...
<<descriptivesObjectiveOutcome, echo=F,eval=T>>=
  ...
  ##rename columns for tables:
  #library(dplyr)
  tlcObjOutcome <- rename(tlcObjOutcome, Name = englishName, Sex = sex, Team = team,
                                  Condition = condition, Time = time, = AverageOutcome TrialOutcomeAvg = outcomeAvg, TrialOutcomeSD = outcomeSd)
...
<<manipulationCheckTable, echo=F,eval=T>>=
  ...
  twcManipulationCheck <- subset(twc, select = c(age, sex, condition, team,
                                      groupConfidentTechnicalChallenges_2, indConfidenceChallenges_2, arousalFactorPre, groupPerformance_3Norm, indPerformance_3Norm, groupClickPostFactor, groupBondingFactorPost))
  ##rename columns for tables:
  library(dplyr)
  twcManipulationCheck <- rename(twcManipulationCheck, Condition = condition,
                                      ConfidenceInGroupAbility = groupConfidentTechnicalChallenges_2, ConfidenceIndividualAbility = indConfidenceChallenges_2, Arousal = arousalFactorPre, GroupPerformVsExpected = groupPerformance_3Norm, IndPerformVsExpected = indPerformance_3Norm, GroupClickPost = groupClickPostFactor, GroupBondingPost = groupBondingFactorPost)
...
It was still possible, however, that the experimental primes influenced dimensions of athlete behaviour dimensions inaccessible through self-report.
...
<<performanceConditionPostTable, echo=F,eval=T>>=
  ...
  twcPerformancePost <- rename(twcPerformancePost, Team = team, Condition = condition,
                                      Sex = sex, Age = age, JointActionSuccess = jointActionSuccessPost, IndividualComponentPerformance = indPerformancePost, GroupPerformVsExpected = groupPerformance_3, IndPerformVsExpected = indPerformance_3)


  ## create a variable list which we want in table one
  listVars <- c("JointActionSuccess", "IndividualComponentPerformance", "GroupPerformVsExpected", "IndPerformVsExpected")
...
<<factorsByCondition, eval=T,echo=F>>=
  ...
  tlcFactors <- rename(tlcFactors, Team = team, Condition = condition, Sex = sex,
                                      ...
                                      IndividualComponentPerformance = indPerformanceFactor, GroupPerformVsExpected = groupPerformanceExpectations, IndividualPerformanceExpectations = indPerformanceExpectations, GroupClick = groupClickFactor1, GroupClickPost = groupClickPostFactor, GroupBonding = groupBondingFactor, TeamBondingFactor = teamBondingFactor, Arousal = arousalFactor)
  ...
  listVars <- c("JointActionSuccess", "IndividualComponentPerformance",
                                      "GroupPerformVsExpected", "IndividualPerformanceExpectations", "GroupClick", "GroupBonding", "GroupClickPost", "TeamBondingFactor", "Arousal")
...
<<fullOutcomeAvgSD, eval=T,echo=F, fig.cap="">>=
  ...
  outcomeDf <- rename(outcomeDf, Condition = condition, Team = team,
                                  Session = sessionName,
...
<<fullOutcomeAvgSessionBoxplot, eval=T,echo=F, fig.cap="">>=
  ...
  outcomeDf$Session <- plyr::revalue(outcomeDf$Session, c("sdwB" = "sdwH",
                                                    "sdmB" = "sdmH",
                                                    "bjwB" = "bjwH", "bjmB" = "bjmH", "sdwA" = "sdwL",  "sdmA" = "sdmL", "bjwA" = "bjwL", "bjmA" = "bjmL"))

  fullOutcomeAvgSdSessionBox <- ggplot(outcomeDf,
                            aes(x=Session, y=AverageOutcome, size=StandardDeviation)) +
                            geom_boxplot() +
                            labs(title = "Plot of avg.performance outcome \n by condition", x = "Experiment Session", y = "Average Performance Outcome") +
...
<<groupPerfExpClickModel, echo=F,eval=T>>=
  ...
  #confint:
  groupPerfExpClick.controls.CIs <- confint.merMod(groupPerfExpClick.controls, method = "Wald")
...
(all
  \cooksD
    \Sexpr{max(groupPerfExpClickCooksD)}
  ,
...
<<groupPerfExpClickAssumptionsHist, echo=F,eval=T>>=
  ...
  hist(groupPerfExpClickResid , freq = FALSE, main = "Distribution of M1 residuals \n (standardised)", xlab = "Residuals")
...
<<groupPerfExpClickAssumptionsScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(groupPerfExpClick.controls, main = "Scatterplot of M1 Residuals \n (standardised)")
  dev.copy(pdf, "TEM1Scatter.pdf")
  dev.off()
@
<<groupPerfExpClickQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(groupPerfExpClick.controls), main = "Norm QQ Plot of M1 Residuals")
...
<<groupPerfExpClickCooksD, echo=F,eval=T>>=
  ...
  groupPerfExpClickCooksD <- cooks.distance(groupPerfExpClick.controls)
  plot(groupPerfExpClickCooksD, main = "M1 Influential cases \n (Cook's Distance)", ylab = "cooks.distance")
...
<<jASgroupPerfExpClickScatterLM, eval=T,echo=F>>=
  ...
  groupPerfExpJASClickScatterLM +
              ggtitle("Positive violation of Team Performance Expectations \n moderates effect of Joint Action Success on Group Click") +
              theme(plot.title = element_text(size=12)) +
              xlab("Joint Action Success") +
              ylab("Group Click") +
              scale_size_continuous(guide = FALSE)+
              geom_abline(aes(intercept=0.16, slope=0.7371, linetype='-1SD Expectation Violation'))+
              geom_abline(aes(intercept=0.2242, slope=0.6815, linetype='Mean Expectation Violation'))+
              geom_abline(aes(intercept=0.2885, slope=0.6259, linetype='+1SD Expectation Violation'))+
              scale_linetype_manual(values=c('dotted','dashed','solid'),
              breaks=c('-1SD ExpectationViolation','Mean Expectation Violation','+1SD Expectation Violation'),name='Simple\nSlope')+
              apatheme
@

<<jASgroupPerfExpClickScatterLMER, eval=T, echo=F, fig.cap="Positive violation of Group Performance Expectations \n moderates the relationship between perceived Joint Action Success and Group Click">>=
  #twc$groupPerformance_3Norm <- scale(twc$groupPerformance_3)
  ...
  jASGroupPerfExpClickScatterLMER +
              theme(plot.title = element_text(size=12)) +
              labs(x = 'Joint Action Success', y = "Group Click") +
              scale_size_continuous(guide=FALSE)+
              geom_abline(aes(intercept=0.0994, slope=0.6809, linetype= "-1SD Expect"))+
              geom_abline(aes(intercept=0.0754, slope=0.4945, linetype= "Mean Expect"))+
              geom_abline(aes(intercept=0.0619, slope=0.3892, linetype="+1SD Expect"))+
              scale_linetype_manual(values=c('dotted','dashed','solid'),
              breaks=c("-1SD Expect", "Mean Expect","+1SD Expect"),name="Group Performance \n Vs Expected") +
              apatheme
@

Figure ~\ref{fig:teamPerfExpClickScatter} shows that the slope of the relationship between between perceptions of Joint Action Success and Group Click appears to be  moderated, such that higher levels (+1 standard deviation) of group performance expectation violation results in a reduction in steepness of the slope.
...
,
  \betaest
    \Sexpr{coef(summary(jASgroupPerfExpClick.int.controls))["jointActionSuccessPost:groupPerformance_3Norm","Estimate"]}
  \CIstart
    \Sexpr{jASgroupPerfExpClick.int.controls.CIs["jointActionSuccessPost:groupPerformance_3Norm", 1]}
  ,
    \Sexpr{jASgroupPerfExpClick.int.controls.CIs["jointActionSuccessPost:groupPerformance_3Norm", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(jASgroupPerfExpClick.int.controls))["jointActionSuccessPost:groupPerformance_3Norm","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(jASgroupPerfExpClick.int.controls))["jointActionSuccessPost:groupPerformance_3Norm","df"]}
    )
  =
    \Sexpr{coef(summary(jASgroupPerfExpClick.int.controls))["jointActionSuccessPost:groupPerformance_3Norm","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(jASgroupPerfExpClick.int.controls))["jointActionSuccessPost:groupPerformance_3Norm","Pr(>|t|)"]}
  ,
  \MR2
    \Sexpr{jASgroupPerfExpClick.int.controls.R2[,5]}
  ,
  \CR2
    \Sexpr{jASgroupPerfExpClick.int.controls.R2[,6]}
,
...
<<jASgroupPerfExpClickIntHist, echo=F,eval=T>>=
  ...
  hist(jASgroupPerfExpClickIntResid, freq = FALSE, main = "Distribution of M1a Residuals", xlab = "Residuals")
...
<<jASgroupPerfExpClick.intAssumptionsCooksD, echo=F,eval=T>>=
  #indluential cases
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
  plot(cooks.distance(jASgroupPerfExpClick.int.controls), main = "M1a Influential Cases \n (Cook's Distance)", ylab = "cooks.distance")
  ...
  library(influence.ME)
  jASgroupPerfExpClickIntCooksD <- cooks.distance(jASgroupPerfExpClick.int.controls)
  max(jASgroupPerfExpClickIntCooksD) # 1.382886e+00 (row 28)
...
<<groupClickBondScatter, eval=F,echo=T>>=
  groupClickBondScatter <- ggplot(twc, aes(x=groupClickPostFactor,
  ...
  #              guides(color = guide_legend(title="Condition"))+
              theme(plot.title = element_text(size=12)) +
              labs(x = "Group Click", y = "Social Bonding", color = "Condition")+
              apatheme
              #              scale_fill_
              #"Perceptions of Group Click \n predict feelings of Social Bonding to training group"
...
<<clickBondingModel, eval=T,echo=F>>=
  ...
  clickBondingCondition.controls <- lmer(groupBondingFactorPost ~
                                                    groupClickPostFactor*condition +
                                          arousalFactorPost + fullOutcomeAvg +
                                          subjectiveCompetenceFactor + objectiveCompetenceFactor +
                                          (groupClickPostFactor|sessionName),
                                          data = twc,
  ...
  summary(clickBondingCondition.controls)  # interaction is not significant and so dropped from analysis
  #CIs:
...
A LMER model (controlling for arousal, average trial performance outcome, and technical competence (subjective and objective)) revealed a strong positive relationship between feelings of group click and feelings of social bonding to the training group,
  ...
  \betaest
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor","Estimate"]}
  \CIstart
    \Sexpr{clickBondingCondition.controls.CIs["groupClickPostFactor", 1]}
  ,
    \Sexpr{clickBondingCondition.controls.CIs["groupClickPostFactor", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor","df"]}
    )
  =
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor","Pr(>|t|)"]}
  ,
  \MR2
    \Sexpr{clickBondingCondition.controls.R2[,5]}
  ,
  \CR2
    \Sexpr{clickBondingCondition.controls.R2[,6]}
.
The interaction effect of group click and condition on social bonding was not significant
,
  \betaest
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor:conditionhigh","Estimate"]}
  \CIstart
    \Sexpr{clickBondingCondition.controls.CIs["groupClickPostFactor:conditionhigh", 1]}
  ,
    \Sexpr{clickBondingCondition.controls.CIs["groupClickPostFactor:conditionhigh", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor:conditionhigh","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor:conditionhigh","df"]}
    )
  =
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor:conditionhigh","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(clickBondingCondition.controls))["groupClickPostFactor:conditionhigh","Pr(>|t|)"]}
,
...
%ASSUMPTIONS:
<<clickBondingConditionHist, echo=F,eval=T>>=
  #1.historgram with density line
  clickBondingConditionResid <- resid(clickBondingCondition.controls, type = "pearson")
  hist(clickBondingConditionResid, freq = FALSE, main = "Distribution of M2 Residuals", xlab = "Residuals")
  lines(density(clickBondingConditionResid, na.rm = TRUE))
  dev.copy(pdf, "TEM2Hist.pdf")
  dev.off()
  #non-normality
  clickBondingConditionResid.Wald <- shapiro.test(clickBondingConditionResid)
  psych::describe(clickBondingConditionResid)
@
<<clickBondingConditionScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(clickBondingCondition.controls, main = "Scatterplot of M2 Residuals")
  dev.copy(pdf, "TEM2Scatter.pdf")
  dev.off()
@
<<clickBondingConditionQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(clickBondingCondition.controls), main = "Norm QQ Plot of M2 Residuals")
  qqline(residuals(clickBondingCondition.controls))
  dev.copy(pdf, "TEM2QQNorm.pdf")
  dev.off()
@
<<clickBondingConditionCooksD, echo=F,eval=T>>=
  #indluential cases
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
  clickBondingCondition.controls.cooksD <- cooks.distance(clickBondingCondition.controls)
  plot(cooks.distance(clickBondingCondition.controls), main = "M2 Influential Cases \n (Cook's Distance)", ylab = "cooks.distance")
...
Model residuals were normally distributed around zero
  (
    \resdist
      \Sexpr{clickBondingCondition.controls.Wald$statistic},
    \pvalue
      \Sexpr{clickBondingCondition.controls.Wald$p.value}
  )
  ...
  \cooksD
  <
    \Sexpr{max(clickBondingCondition.controls.cooksD)}, see model assumptions in Appendix  ~\ref{fig:M2Assumptions}
  )
  .
...
<<groupPerfExpBondConditionScatter, eval=T,echo=F, fig.cap= 'Positive violation of group performance expectations \n predict feelings of Social Bonding to training group'>>=
  groupPerfExpBondScatter <- ggplot(twc, aes(x=groupPerformance_3Norm,
                              y=groupBondingFactorPost, color=condition)) + geom_point(shape = 1) +
...
<<groupPerfExpBondingModel, eval=T,echo=F>>=
  ...
  groupPerfExpBonding.intercept <- lmer(groupBondingFactorPost ~ 1 + ( 1 |sessionName),
                              data = twc,
                              REML = F)
  groupPerfExpBondingCondition.main <- lmer(groupBondingFactorPost ~
                                          groupPerformance_3Norm*condition +
                                          (groupPerformance_3Norm |sessionName),
                                          data = twc,
  ...
  groupPerfExpBondingCondition.controls <- lmer(groupBondingFactorPost ~
                                            groupPerformance_3Norm*condition  +
                                            indPerformance_3Norm + arousalFactorPost + fullOutcomeAvg +
                                            subjectiveCompetenceFactor + objectiveCompetenceFactor +
                                            (groupPerformance_3Norm |sessionName),
                                            ...
                                            data = twc,
                                            REML = F)
  summary(groupPerfExpBondingCondition.controls)
  #CIs:
...
A LMER model with group performance expectation violation, condition, and their interaction as fixed effects, experiment session as random effect (correlated slope and intercept), and controls, revealed a significant positive relationship between the interaction of group performance expectation violation and condition on social bonding
  ,
  \betaest
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm:conditionhigh","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["groupPerformance_3Norm:conditionhigh", 1]}
  ,
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["groupPerformance_3Norm:conditionhigh", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm:conditionhigh","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm:conditionhigh","df"]}
    )
  =
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm:conditionhigh","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm:conditionhigh","Pr(>|t|)"]}
  ,
  \MR2
    \Sexpr{groupPerfExpBondingCondition.controls.R2[,5]}
  ,
  \CR2
    \Sexpr{groupPerfExpBondingCondition.controls.R2[,6]}
  .
The main effects of group performance expectation violation
  (
  \betaest
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["groupPerformance_3Norm", 1]}
  ,
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["groupPerformance_3Norm", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm","df"]}
    )
  =
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["groupPerformance_3Norm","Pr(>|t|)"]}
  )
  and experiment condition
  (
  \betaest
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["conditionhigh","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["conditionhigh", 1]}
  ,
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["conditionhigh", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["conditionhigh","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["conditionhigh","df"]}
    )
  =
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["conditionhigh","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["conditionhigh","Pr(>|t|)"]}
  )
were not significant predictors of social bonding.  Average performance in each trial was, however, a significant predictor of social bonding
  ,
  \betaest
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["fullOutcomeAvg","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["fullOutcomeAvg", 1]}
  ,
    \Sexpr{groupPerfExpBondingCondition.controls.CIs["fullOutcomeAvg", 2]}
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["fullOutcomeAvg","Std. Error"]}
  ,
  t(
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["fullOutcomeAvg","df"]}
    )
  =
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["fullOutcomeAvg","t value"]}
  ,
  \pvalue
    \Sexpr{coef(summary(groupPerfExpBondingCondition.controls))["fullOutcomeAvg","Pr(>|t|)"]}
  .
...
<<groupPerfExpBondingConditionHist, echo=F,eval=T>>=
  #1.historgram with density line
  groupPerfExpBondingConditionResid <- resid(groupPerfExpBondingCondition.controls, type = "pearson")
  hist(groupPerfExpBondingConditionResid, freq = FALSE, main = "Distribution of M3 Residuals", xlab = "Residuals")
  lines(density(groupPerfExpBondingConditionResid, na.rm = TRUE))
  dev.copy(pdf, "TEM3Hist.pdf")
  dev.off()
  #non-normality
  groupPerfExpBondingConditionResid.Wald <- shapiro.test(groupPerfExpBondingConditionResid)
  psych::describe(groupPerfExpBondingConditionResid)
@
<<groupPerfExpBondingConditionScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(groupPerfExpBondingCondition.controls, main = "Scatterplot of M3 Residuals")
  dev.copy(pdf, "TEM3Scatter.pdf")
  dev.off()
@
<<groupPerfExpBondingConditionQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(groupPerfExpBondingCondition.controls), main = "Norm QQ Plot of M3 Residuals")
  qqline(residuals(groupPerfExpBondingCondition.controls))
...
<<groupPerfExpBondingConditionCooksD, echo=F,eval=T>>=
  #indluential cases
  #cook's distances all < 1, so no cause for concern, (Field 2012: 271)
  groupPerfExpBondingCondition.controls.cooksD <- cooks.distance(groupPerfExpBondingCondition.controls)
  plot(cooks.distance(groupPerfExpBondingCondition.controls), main = "M3 Influential Cases \n (Cook's Distance)", ylab = "cooks.distance")
...
and all \cooksD were less than
  \Sexpr{max(jASgroupPerfExpClickIntCooksD)}
(
...
<<groupPerfExpClickBondMediationModel, eval=T,echo=F>>=
  ...
  summary(m3 <- lmer(groupBondingFactorPost ~
                                      groupPerformance_3Norm*groupClickPostFactor +
                                      arousalFactorPost + fullOutcomeAvg +
  ...
  med.out.groupPerfExp.unclass <- unclass(med.out.groupPerfExp)
@
...
Results of the mediation analysis revealed significant average indirect effect of Group Performance Expectation Violation on Social Bonding attributable to Team Click,
  \betaest
    \Sexpr{coef(summary(med.out.groupPerfExp))["ACME (average)","Estimate"]}
  \CIstart
    \Sexpr{coef(summary(med.out.groupPerfExp))["ACME (average)","95% CI Lower"]}
  ,
    \Sexpr{coef(summary(med.out.groupPerfExp))["ACME (average)","95% CI Upper"]}
  \CIfinish
  ,
  \pvalue
    \Sexpr{coef(summary(med.out.groupPerfExp))["ACME (average)","p-value"]}
  ,
  .
When controlling for the effect of team click on social bonding, the average direct effect between Joint Action Success and Social Bonding was no longer significant, \Sexpr{} (see Figure ~\ref{fig:groupPerfExpClickMediationPlot}).

These results suggest that feelings of team click fully mediate the relationship between group performance expectation violation and and social bonding.
...
<<groupPerfPrePostVariable, eval=T,echo=F>>=
  ...
  tlc$groupPerformancePrePost <- tlc$groupPerformanceExpectations
  tlc$groupPerformancePrePost[tlc$time == 2] <- tlc$groupConfidentTechnicalChallenges[tlc$time == 2]
  ##standardise groupPerformancePrePost in order to align with other predictor variable factors:
  tlc$groupPerfPPNorm <- scale(tlc$groupPerformancePrePost)
  hist(tlc$groupPerfPPNorm)
@

<<groupPerfPrePostPlot, eval=T,echo=F, fig.cap= "Pre-Experiment confidence in group performance and post-Experiment perceptions of group performance versus expectations">>=
    groupPerfPPBox <- ggplot(tlc,
                            aes(x=time, y=groupPerfPPNorm, color=condition)) +
                            geom_boxplot() +
...
<<groupPerfPrePostExpModel, eval=T,echo=F>>=
  ##is there a change in this variable over time?
  summary(groupPerfPrePost.lmer <- lmer(groupPerfPPNorm ~ time
                                      + (1 | englishName)
                                      + (1 | sessionName)
                                      , data = tlc
                                      , REML = FALSE
  ...
  summary(groupPerfPrePostCondition.lmer <- lmer(groupPerfPPNorm ~
                                                time*condition
                                      + (1 | englishName)
                                      + (1 | sessionName)
                                      , data = tlc
...
## use groupClickFactor1, has both time points:
  clickPPBox <- ggplot(tlc,
                          aes(x=time, y=groupClickFactor1, color=condition)) +
                          geom_boxplot() +
                          labs(x = "Time", y = "Group Click") +
                          apatheme
  clickPPBox
@
...
<<clickPrePostModel, eval=T,echo=F>>=
  ...
  ## interaction with condition? <- doesn't look like it
  summary(clickPrePostCondition.lmer <- lmer(groupClickFactor1 ~
                                        time*condition*groupPerfPPNorm
                                      + arousalFactor + fullOutcomeAvg
                                      + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                      + (1 | englishName)
                                      + (1 | sessionName)
                                      , data = tlc
...
## use groupBondingFactor, has both time points:
  bondingPPBox <- ggplot(tlc,
                          aes(x=time, y=groupBondingFactor, color=condition)) +
                          geom_boxplot() +
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% performance, click, and bonding all drop over time: what (if anything) causes this drop?

%1. Does a change in group performance predict a change in click?
<<perfClickTimePlot, eval=F,echo=T, fig.cap="Positive violation of Team Performance Expectations \n predicts Group Click">>=

  perfClickTimeScatter <- ggplot(tlc,
                            aes(x=time, y=groupClickFactor1,
                            size=groupPerfPPNorm)) +
                            geom_point(alpha=.1) +
                            geom_smooth(method=lm, se=FALSE) +
                            labs(x= "Time", y= "Group Click") +  # Don't add shaded confidence region
                            apatheme
  perfClickTimeScatter
@



<<perfClickTime, eval=F,echo=T>>=
summary(perfClickTime.lmer <- lmer(groupClickFactor1 ~ groupPerfPPNorm*time
                                    + (1 | englishName)
                                    + (1 | sessionName)
                                    , data = tlc
                                    , REML = FALSE
...
<<prePostBonding, eval=T,echo=F>>=
tlc$time <- factor(tlc$time,
                    levels = c("1", "2", "3"), ordered = is.ordered(tlc$time))
tlc$condition <- factor(tlc$condition,
                    levels = c("low", "high"), ordered = is.ordered(tlc$condition))
#bondingTime
summary(bondingTime.lmer <- lmer(
                                      groupBondingFactor ~ time +
                                    ( time | sessionName),
                                    data = tlc,
                                    REML = FALSE,
...
#bondingTimeCondition
summary(bondingTimeCondition.lmer <- lmer(
                                      groupBondingFactor ~ time*condition +
                                      (time + condition | sessionName),
                                      data = tlc,
                                      REML = FALSE,
                                      control = lmerControl(optimizer = "optimx",
                                      calc.derivs = FALSE,  optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)
