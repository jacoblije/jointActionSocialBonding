...
In summary, the theory of thermodynamic cognition suggests that humans' behavioural interactions with the environment can be understood are constrained by the ultimate mandate of free energy minimisation.  This theoretical framework is important for understanding the diverse range of component mechanisms that enable joint action, as well as the psychological and social effects of participation in joint action.

\subsubsection{Motor Control}
Organismic regulation with the environment depends crucially on an organism's capacity to move from its current state to a desired state.  In the case of humans, this regulation hinges on motor control \citep{Wolpert1995}.  Theories of motor control generally agree that in order to achieve motor control, the brain generates ``forward models'' that anticipate perceptual inputs and desired motor states \citep{Pickering2014}. Initial theories of motor control suggested that forward models operated via a special-purpose mechanism auxiliary to core mechanisms of perception of action \citep{Wolpert1997}. In this hypothesis, known as the Auxiliary Forward Models (AFM) \citep{Pickering2014}, a copy (i.e., and ``efference copy'') of each desired motor command generated by the brain (known as an ``inverse model'') is sent first to a forward model which then generates predictions about action and perception by simulating musculoskeletal system and contextual environment \citep{Wolpert1995,Blakemore1998,Flanagan2003}. Predictions errors arising from a discrepancy between predicted and resultant sensory inputs are then used as feedback to inform the generation of the next motor command, and so on in a loop, until the fit between sensory prediction and input is sharpened.

An alternative proposal is that that forward models, instead of being auxiliary to, may instead lie at the heart of all forms of perception and action \citep{Friston2010}.  In contrast to AFM, the ``Integrative Forward Models'' account of motor control \citep[IFM, see][]{Pickering2014} posits that predictions from forward models act directly as action commands.  There is no distinct or dedicated mechanism used to predict the outcomes of our own (or others’) actions.  Instead, generative action-oriented models are treated as the actual state of affairs, causing a cascade of downward predictions about what should be the state of the models in the levels below.  In the IFM approach, there is no need for a motor command or efference copy at each level of action prediction, and prediction errors directly update the action models themselves.

Both the AFM and IFM approaches agree that prediction, error minimisation, and hierarchical modelling are core processes in human cognition. From a thermodynamic standpoint, however, IFM appears to offer a more efficient model for free energy minimisation.  By replacing motor commands with direct predictions about proprioceptive consequences, the need for a distinct optimal control calculation (i.e. an inverse model) disappears and along with it the need for an efference copy of the motor command, thus implicitly minimising various energetic costs \citep{Pickering2014,Friston2010}.   Instead, IFMs posit a more complex (distributed) forward model mapping prior beliefs about desired trajectories to sensory consequences.  Whereas the ``heavy lifting'' in AFMs required the use of an efference copy, inverse models, and optimal controllers, in IFM this work is done by the acquisition and use of a more complex predictive (generative) model \citep{Pickering2014}.
...
In recent years,
* psychological (Prinz, 1997, 2013),
* neurophysiological (Rizzolatti and Craighero, 2004; Rizzolatti and Sinigaglia, 2010) and
* computational (Wolpert and Kawato, 1998; Wolpert and Ghahramani, 2000)
accounts have suggested that action perception and action execution are intrinsically coupled in the human brain.

sensory-motor coupling emerges as a result of motor learning: having only visual (Candidi et al., 2012) or auditory (Lahav et al., 2007) experience with a given action is not sufficient to trigger these motor responses—active motor learning is necessary.

This wave culminated in Prinz’s (1990) proposal that perception and action are coded in a common representational domain, and are therefore linked by shared neural resources. common-coding
...
Novembre2014:
(1) generating predictions of our own as well as others’ (i.e., observed) actions; and

  Maidhof et al. (2009) and Ruiz et al. (2009) conducted two similar EEG studies in which they examined the ERPs preceding the execution of piano errors.
      * errors were detected prior to their execution
      * Thus, the coupling of sensory and motor cortices is a dynamical process with a strong anticipatory character that, given the existence of an association between movements and their ensuing effects, permits the generation of predictions about the state of our own body and the sensory consequences of our movements.



(2) as a resource for the co-representation of and coordination with other musicians in the context of an interaction (or joint action)

Several mechanisms may support the prediction and integration of a co-actor’s actions with an actor’s own actions. One of these is the overlap in functional (e.g. Jeannerod, 1999, 2003; Prinz, 1997) and neural (e.g. Rizzolatti & Craighero, 2004; Rizzolatti & Sinigaglia, 2010) mechanisms for action production and action observation.
    * Due to this overlap, people may rep- resent their own and others’ actions in a commensurable format.


* The studies by Drost and colleagues demonstrate that auditory perception primes action if strong action-perception links have been established through instrument-specific training
* Analogous conclusions can be drawn from studies testing the interference between passive music perception and general (i.e., not musical) actions.
   Taylor and Witt (2014) had pianists and novices listening to task-irrelevant ascending or descending scales while making left or right arm movements
   pianists but not the novices responded faster when the direction of their response was compatible with the direction of the musical scale heard in the background

* Keller and Koch (2006a) mental images of anticipated action effects can prime responses to a similar degree as is observed with congruent and incongruent sounds, highlighting the role of action-perception coupling in action preplanning (i.e., before sounds are actually perceived).

* Further studies using a version of the paradigm that required taps to be produced at a specific tempo (rather than rapidly) demonstrated that action-perception coupling does not only
    *  enhance the efficiency of action planning, but also
    *  facilitates timing accuracy and economical force control by optimizing movement kinematics (Keller and Koch, 2006b; Keller et al., 2010)

demonstrating that action- perception coupling in the musicians’ brain is multimodal (Hasegawa et al., 2004; Haslinger et al., 2005; Engel et al., 2012; Candidi et al., 2012)
