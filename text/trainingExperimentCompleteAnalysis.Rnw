...
<<dfAthleteSummary, echo = FALSE, eval = TRUE>>=
...
@
%%OBJECTIVE OUTCOME:
<<descriptivesObjectiveOutcome, echo = FALSE, eval = TRUE>>=
...
<<groupPerfExpJASClickScatterLMER, eval=T, echo=F, fig.cap="Positive violation of Group Performance Expectations \n moderates the relationship between perceived Joint Action Success and Group Click">>=
  #twc$groupPerformance_3Norm <- scale(twc$groupPerformance_3)
  library(lmerTest)
...
Figure ~\ref{fig:jASGroupPerfExpClickScatter} displays the relationship between perceptions of Joint Action Success and Group Click, moderated by perceptions of Team Performance expectations.

Figure ~\ref{fig:teamPerfExpClickScatter} shows a strong positive correlation between violations of prior expectations concerning team performance and feelings of ``click'' with the training group post-Experiment.  The slope of this relationship appears to vary according to condition, as indicated on the scatterplot by the two non-parallel lines of best fit.  A LMER was used to model this relationship: Group Performance Expectations, Condition, and their interaction were included as fixed effects, and experiment session was modelled as a random effect (intercept and slope).  The model also controlled for perceptions of individual performance relative to prior expectations, post-Experiment arousal, the average performance outcome of each invasion drill trial, and subjective and objective measures of athlete technical competence.

The model revealed a signigicant positive main effect of group performance expectation violation on feelings of ``team click'' with the training group, \Sexpr{}.  The main effect of condition, \Sexpr{}, and the interaction effect of group performance expectation violation and condition on click was also significant, \Sexpr{}.  In addition, average performance outcome of each invasion drill trial was also a significant positive predictor of team click, \Sexpr.  These results suggested that group performance expectation violations were a positive predictor of feelings of group click among athletes, and that this relationship was moderated by experiment condition, such that expectation violation had a greater effect on feelings of group click in the high difficulty condition than the low difficulty condition.  Model residuals were normally distributed around zero, W = 0.99001, p-value = 0.9192
...
%ASSUMPTIONS:
<<groupPerfJASExpClickHist, echo=F,eval=T>>=
  #1.historgram with density line
...
\myparagraph{Prediction 2: Feelings of group click will positively correlate with feelings of social bonding to the group}
...
@
<<groupPerfExpClickMediationPlot, fig.cap =''>>=
