...
A total of 8 experimental sessions were conducted, with two of these sessions involving only six athletes (rather than eight), and a further two sessions involving a dummy participant who did not participate in the survey responses before or after the session.  The survey and video data of the remaining 58 athletes were analysed.
...
twcAthleteInfo$teamStatus <- ifelse(!is.na(twcAthleteInfo$teamStatus) &
                                    twcAthleteInfo$teamStatus == 5, "Full time employee",
                                      ifelse(!is.na(twcAthleteInfo$teamStatus) & twcAthleteInfo$teamStatus == 4, "Fully contracted",
...
twcAthleteInfo$team <ifelse(twcAthleteInfo$team == "bjm", "Beijing Men",
                                    ifelse(twcAthleteInfo$team == "bjw", "Beijing Women",
                                    ifelse(twcAthleteInfo$team == "sdm", "Shandong Men",
                                    ifelse(twcAthleteInfo$team == "sdm", "Shandong Women", NA)))



##rename columns for tables:
library(dplyr)
twcAthleteInfo <- rename(twcAthleteInfo, Position = startingReserve, TrainingAge = trainingAge, YearsInTeam = yearsTeam, AthleteLevel = athleteStatus, ContractStatus = teamStatus, Team = team, Condition = condition, Sex = sex, Age = age)


## create a variable list which we want in table one
listVars <- c("Age", "Sex", "Condition", "Team", "Position",  "TrainingAge" , "YearsInTeam", "AthleteLevel", "ContractStatus")
...
print(xtable(table1_overall, type="latex", caption = "Overview of experimental sample"),
...
tlcCompPerfSubj <- rename(tlcCompPerfSubj, Name = englishName, Sex = sex, Team = team,
                                Condition = condition, Time = time, CompetenceVsTeammates = indAbilityTeammates_1, CompetenceVsChinesePros = indAbilityChina_1, CompetenceVsInternationalPros = indAbilityInternational_1, IndPerformanceConfidence =  indPerformanceOnConfidence_1, IndPerformanceMood = indPerformanceOnMood_1)


# create high and low condition dfs:
tlcCompPerfSubjHigh <- subset(tlcCompPerfSubj, Condition == "high")

tlcCompPerfSubjLow <- subset(tlcCompPerfSubj, Condition == "low")

## create a variable list which we want in table one
listVars <- c("CompetenceVsTeammates", "CompetenceVsChinesePros",
                "CompetenceVsInternationalPros", "IndPerformanceConfidence", "IndPerformanceMood")
...
## print to latex file in wd
## this latex file will later be inputted
print(xtable(tableTlcCompPerfSubj.condition, type="latex", caption = "Athlete subjective perceptions of \n individual technical competence by condition"), file="compPerfSubjTraining.tex")
...
GROUP PERFRORMANCE:
 [25] "groupAttackingLine"       "groupDefensiveLine"
 "groupSupportPlay"

  [29] "groupConfidentTechnicalChallenges"

  [35] "groupOnfieldCommunication"         "groupPerformance"
  [37] "groupPerformanceFactor"

TEAM PERFORMANCE:
        [87] "teamPerformance_1"
  "teamAbilityChina_1"
[11]
    "teamSupportPlay_1"

        [77] "teamAttackLine_1"
        "teamOnfieldCommunication_1"
        [83] "teamDefensiveLine_1"
        ...
        GROUP CLICK:
         [27] "groupClickFactor"
          "groupClickPictorial"
           [43] "groupUnspokenUnderstanding"
                       [65] "reliabilityForOthers_1"            "reliabilityOfOthers_1"
                [33] "groupGeneralAtmosphere"
                "groupReliabilityForOthers"
                [39] "groupReliabilityOfOthers"

        GROUP BONDING:
                       "groupBondingFactor"
          [31] "groupEmotionalSupport"     "groupFusionPictorial"   "groupSharedGoal"
        "groupId"
                         [41] "groupStayChange"

         [17] "fullOutcomeAvg"                    "fullOutcomeSd"
         [61] "rankCountry"                       "rankFamily"
         [63] "rankTeam"
...
TEAM  BONDING:
     [13] "emotionalSupport"        "sharedGoal"
     [19] "fusionPictorialCountry"            "fusionPictorialFamily"
     [21] "fusionPictorialTeam"               "fusionVerbal"
 [79] "teamBondingFactor"
...
"teamPunctuality"
                "teamAttendanceMeals"
                [85] "teamGeneralConduct"
                "teamCurfew"
                  "teamDisciplineFactor"

PERSONALITY:
                             "tipiAgreeableness"
 [93] "tipiConscientiousness"             "tipiEmotionalStability"
 [95] "tipiExtraverted"                   "tipiOpenness"
...
2. performance  (including performance on mood)
3. click
...
Athletes were asked two questions before the experiment to gauge their confidence in individual and group ability to handle the technical challenges of the training drill. Athlete responses to these items were expected to vary according to condition, such that athletes in the high difficulty condition were less confident about their own and their group's ability to meet the technical challenges of the training session. In addition, it was predicted that athlete arousal, measured in the survey immediately prior to each experiment session, would also vary according to condition, such that athletes in the high difficulty condition were more aroused than athtletes in the low difficulty condition.  Meaningful variation in these variables could be used as evidence that the experimental manipulation was successful.
