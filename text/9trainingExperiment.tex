...
Study predictions were tested by analysing two data sets:  1) Post-Drill data and 2) variation between Pre- and Post-Drill data.  Results for each data set are reported under each study prediction in Section ~\ref{sect:resultsStudyPredictions}.
...
\myparagraph{Group Performance relative to prior expectations}
...
\subsection{Factors Pre- to Post-Drill}
...
LMER models controlling for objective and subjective levels of technical competence (fixed) and experiment session (random), revealed no overall condition-wise differences in athlete confidence in group ($\betavec .008 \CIstart -.30, .31 \CIfinish \SE .16, t(54)= .05, \pvalue .96$) or individual ability to meet the technical challenges of the drill ($\betavec .06 \CIstart -.30, .42 \CIfinish \SE .18, t(54)= .33, \pvalue .74$). Pre-Drill measures of arousal also did not differ according to condition ($\betavec .07 \CIstart -.29, .44 \CIfinish \SE .19, t(53)= .38, \pvalue .71$).

Based on these results, there was no explicit evidence to suggest that the experimental prime had successfully manipulated athlete expectations around performance or arousal prior to the Drill.  For a more detailed review of manipulation checks, see Appendix ~\ref{app9:trainingExperiment} Section ~\ref{app9:manipulationChecks}.
...
\subsubsection{Post-Drill survey}
 ...
 According to LMER models, predictor variables (group performance relative to prior expectations, individual performance relative to prior expectations, team performance components, or individual performance components) showed no significant variation according to experiment condition (see Appendix ~\ref{app9:trainingExperiment} Section ~\ref{app9:conditionDifferencesIV} for a full description results).

 Similarly, LMER models revealed that outcome variables (team click, group bonding, and bonding to team according to condition) did not vary by condition (see Appendix ~\ref{app9:trainingExperiment} Section ~\ref{app9:manipulationChecksDV} for a full description of results).
...
According to LMER models, predictor variables (change in group performance relative to prior expectations, change in individual performance relative to prior expectations, change in team performance components, or change in individual performance components) showed no significant variation according to experiment condition (see Appendix ~\ref{app9:trainingExperiment} Section ~\ref{app9:conditionDifferencesIV} for a full description results).
...
These results suggest that there was no clearly observable variation in athlete self-report scores according to condition.  As such, survey responses were collapsed into one overall sample, and condition was introduced as an interaction effect in subsequent LMER models.
...
\subsection{Analysis of Study Predictions\label{sect:resultsStudyPredictions}}
...
Results of the Post-Drill survey supported this prediction. There was a significant main effect group performance vs expectations on feelings of team click with the training group ($\betavec .57 \CIstart .23, .90 \CIfinish \SE .17, t(5.40) = 3.21, \pvalue .02$ $marginal R^2 = .53$, $conditional R^2 = .60$), indicating that more positive perceptions of group performance relative to prior expectations predicts feelings of team click.  All other main effects were not significant.
...
Results of the Pre- to Post-Drill survey also supported this prediction.  There was a significant main effect of change in perceptions of group performance on change in team click ($\betavec .54 \CIstart .30, .78 \CIfinish \SE .12, t(53) = 4.371, \pvalue .00006 \MR \CR $), indicating that greater positive change in perceptions of group performance relative to prior expectations predicts positive change in feelings of team click.  All other main effects were not significant.
...
Results of the Post-Drill survey supported this prediction. There was a significant main effect of feelings of team click on feelings of social bonding to the training group ($\betavec .63 \CIstart .46, .79 \CIfinish \SE .09, t(53) = 7.28, p < .0001$), indicating that stronger feelings of team click predict stronger feelings of social bonding.  All other main effects of the model were not significant.
...
Model residuals were normally distributed around zero ($\resdist .96, \pvalue .08$), and Cook's Distances were less than  the model did not to contain any observations that unjustifiably influenced parameter estimates (all $\cooksD < .5$), but the distribution of residuals and QQ PLot did show a clear positive outlier (for full presentation of model assumptions, see Figure ~\ref{fig:M22Assumptions} in Appendix ~\ref{app9:trainingExperiment} Section ~\ref{app9:prePostExperimentModelAssumptions}).

W = 0.96055, p-value = 0.07764
...
\subsection{Analysis of Pre-post Survey Data}
...
\myparagraph{Prediction 4: Change in feelings of team click
