...
<<descriptivesObjectiveOutcome, echo=F,eval=T>>=
  ...
  tlcObjOutcome <- dplyr::rename(tlcObjOutcome, Name = englishName, Sex = sex, Team = team,
                                  Condition = condition, Time = time, AverageOutcome = fullOutcomeAvg, TrialOutcomeAvg = outcomeAvg, TrialOutcomeSD = outcomeSD)
  ## create a variable list which we want in table one
...
<<indConfidenceVariance, echo=F,eval=T>>=
 ...
 # variance appears to differ significantly
  indPerfBartlett <- stats::var.test(high$indConfidenceChallenges_2, low$indConfidenceChallenges_2)
  # variance appears to differ significantly
...
An analysis of arousal by condition also failed to reveal meaningful variation by condition, (see Table ~\ref{tab:manipulationCheckTable} and Figure ~\ref{fig:arousalFactorPreBoxPlot}). Judging by the boxplot and histogram of each condition (see Figures ~\ref{fig:histArousalFactorPreHigh} and ~\ref{fig:histArousalFactorPreLow}), the variance of arousal in the high difficulty condition did appear to be higher than the low difficulty condition. A Bartlett's test for homogeneity of variance, however, revealed that the difference was not significant, \Sexpr{arousalFactorPreBartlett}.
...
The model revealed a significant positive main effect of group performance expectation violation on feelings of team click with the training group,
  ...
  \CIfinish
  \SE
    \Sexpr{coef(summary(groupPerfExpClick.controls))["groupPerformance_3Norm",2]}
  ,
  t(
...
(
  \betavec
    \Sexpr{coef(summary(groupPerfExpClick.controls))["conditionhigh","Estimate"]}
  \CIstart
    \Sexpr{groupPerfExpClick.controls.CIs["groupPerformance_3Norm", 1]}
  ,
    \Sexpr{groupPerfExpClick.controls.CIs["groupPerformance_3Norm", 2]}
...
<<jASgroupPerfExpClickScatterLM, eval=T,echo=F>>=
  ...
  groupPerfExpJASClickScatterLM +
              ggtitle("Positive violation of Team Performance \n Expectations moderates effect of \n Joint Action Success on Group Click") +
              theme(plot.title = element_text(size=12)) +
              ...
              breaks=c('-1SD PerfExpect','Mean PerfExpect','+1SD PerfExpect'),name='Simple\nSlope')+
              apatheme
@
...
<<clickBondingConditionHist, echo=F,eval=T>>=
  ...
  dev.off()
  #non-normality
  clickBondingCondition.controls.Wald <- shapiro.test(clickBondingConditionResid)
...
#  library(lmerTest)
  ...
  summary(m3 <- lmer(groupBondingFactorPost ~
                                      ...
                                      data = twc, REML = FALSE, control = lmerControl(optimizer = "optimx", 
  ...
  #tidy(summary(med.out.groupPerfExp))
  med.out.groupPerfExp.unclass <- unclass(summary(med.out.groupPerfExp))
  summary(med.out.groupPerfExp.unclass)
@
<<groupPerfExpClickMediationPlot, eval=T,echo=F>>=
  plot(med.out.groupPerfExp, main = "Group Click partially mediates relationship \n between team performance expectation \n violation and social bonding")
@
