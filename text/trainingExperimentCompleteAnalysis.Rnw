...
\subsubsection{Team Click and Social Bonding to the team}
...
Items concerning bonding to the team (emotional support, shared goal, fusion pictorial, fusion verbal, and group identification) were subjected to EFA.  Correlations between items were all reasonably high (all $r's > \Sexpr{round(min(groupBondingMatrix), digits = 2)}>$), which suggested that one factor would be appropriate (see Table ee Table ~\ref{tab:groupBondingCorrTable}). The KMO index and Bartlett's test both suggested high sampling adequacy, (KMO =  \Sexpr{round(KMOgroupBondingMatrix$MSA, digits = 2)}, \chi^2(\Sexpr{bartlettGroupBonding$df}, N = \Sexpr{groupBondingSampleSize}) = \Sexpr{bartlettGroupBonding$chisq}, p = \Sexpr{round(bartlettGroupBonding$p.value, digits = 2)). One factor, labelled ``Group Social Bonding'' was imposed on the data, which explained \Sexpr{prVar.groupBondingFA} \% of the overall variance (SS Loading = \Sexpr{SSLoadings.groupBondingFA}). $Guttmans \lambda = \Sexpr{round(reliabilityGroupBonding$total[,3], digits = 2)}$ and $Cronbachs \alpha = \Sexpr{round(reliabilityGroupBonding$total[,2], digits = 2)}$ indicated that the data reduction was appropriate and reliable.
...
names(tlc)[names(tlc) == "Factor1"] <- "subjectiveCompetenceFactor"
...
#----------------------FACTOR ANALYSIS: TEAM -------------------------#

head(tlc)
boxplot(rankFamily ~ tlc$time, data = tlc)


#BONDING: emoSupport, sharedGoal, fusionPictor
# Run PCA to determine number of factors
tlcTeamBonding <- subset(tlc, select = c(emotionalSupport, sharedGoal, fusionPictorialTeam))
# excluding: fusionPictorialCountry, fusionPictorialFamily, fusionVerbal, groupId
tlcTeamBonding.pca <- princomp(na.omit(tlcTeamBonding))
summary(tlcTeamBonding.pca)  #<- looks like 1 or 2 factors
plot(tlcTeamBonding.pca)
# Run EFA for TeamBonding

tlcTeamBonding.fa <- factanal(~ emotionalSupport + sharedGoal + fusionPictorialTeam, 1,
                                       data = tlc, rotation = "promax", na.action = na.exclude, scores = "regression")  #<- this formula format allows preserving of NAs for later rebind to df
# + fusionPictorialCountry + fusionPictorialFamily +
tlcTeamBonding.fa
head(tlcTeamBonding.fa$scores)
...
names(tlc)[names(tlc) == "Factor1"] <- "teamBondingFactor"
teamBondingMatrix <- cor(tlcTeamBonding, use = "complete")
KMO(teamBondingMatrix)
cortest.bartlett(teamBondingMatrix, n = 116)



# CLICK:  reliabilityForOthers, reliabilityOfOthers, abilityExtended, clickPictorial, unspokenUnderstanding, generalAtmosphere


tlcTeamClick <- subset(tlc, select = c(abilityExtended, unspokenUnderstanding, generalAtmosphere))

tlcTeamClick.pca <- princomp(na.omit(tlcTeamClick))
summary(tlcTeamClick.pca)  #<- looks like 1 or 2 factors
plot(tlcTeamClick.pca)
# Run EFA for teamClick

tlcTeamClick.fa <- factanal(~ abilityExtended + unspokenUnderstanding + generalAtmosphere, 1,
                                     data = tlc, rotation = "promax", na.action = na.exclude, scores = "regression")  #<- this formula format allows preserving of NAs for later rebind to df
tlcTeamClick.fa
...
KMO(teamClickMatrix)
cortest.bartlett(teamClickMatrix, n = 117)
...
# DISCIPLINE:  teamAttendanceMeals	teamCurfew	teamGeneralConduct	teamPunctuality
...
summary(tlcTeamDiscipline.pca)  #<- looks like 1 or 2 factors
plot(tlcTeamDiscipline.pca)
# Run EFA for teamDiscipline

tlcTeamDiscipline.fa <- factanal(~ teamAttendanceMeals + teamCurfew + teamGeneralConduct + teamPunctuality, 1,
                                          data = tlc, rotation = "promax", na.action = na.exclude, scores = "regression")  #<- this formula format allows preserving of NAs for later rebind to df
tlcTeamDiscipline.fa
...
KMO(teamDisciplineMatrix)
cortest.bartlett(teamDisciplineMatrix, n = 117)

write.csv(tlc, "tlc.csv", na = "")
write.csv(training, "training.csv", na = "")
@

<<manipulationChecks>>=
#--------------------------Manipulation Checks-------------------------------------------#

#condition differences in: groupConfidentTechnicalChallenges

#groupConfidentTechnicalChallenges
hist(trainingLong$groupConfidentTechnicalChallenges[trainingLong$condition == "low"])
hist(trainingLong$groupConfidentTechnicalChallenges[trainingLong$condition == "high"])
t.test(trainingLong$groupConfidentTechnicalChallenges[trainingLong$condition == "high"], trainingLong$groupConfidentTechnicalChallenges[trainingLong$condition == "low"], paired = T)

#indConfidenceChallenges
hist(trainingLong$indConfidenceChallenges[trainingLong$condition == "low" & trainingLong$time == 2])
hist(trainingLong$indConfidenceChallenges[trainingLong$condition == "low" & trainingLong$time == 3])
hist(trainingLong$indConfidenceChallenges[trainingLong$condition == "high"])
hist(trainingLong$indConfidenceChallenges[trainingLong$condition == "high" & trainingLong$time == 2])
hist(trainingLong$indConfidenceChallenges[trainingLong$condition == "high" & trainingLong$time == 3])
t.test(trainingLong$indConfidenceChallenges[trainingLong$condition == "high"], trainingLong$groupConfidentTechnicalChallenges[trainingLong$condition == "low"], paired = T)
@
