...
1) perceptions of components and overall individual and team performance in joint action,
...
%(Appendix Tables ~\ref{tab:indPerfTimeLowTraining} and ~\ref{tab:indPerfTimeHighTraining}), group (Appendix Tables ~\ref{tab:groupPerfTimeLowTraining} and ~\ref{tab:groupPerfTimeHighTraining}),
%and team (measured at Baseline only, see Appendix Table ~\ref{tab:teamPerfTimeBaselineTraining})
...
%(see Appendix Tables ~\ref{tab:groupClickTimeHighTraining}\nobreakdash--\ref{tab:teamBondingTimeHighTraining}).
...
%(see Appendix Table ~\ref{tab:objectiveOutcomeCondition}).
...
\begin{figure}
    ...
    \caption{More positive perceptions of Team Performance relative to prior expectations predict Group Click}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\linewidth,keepaspectratio] {images/jASgroupPerfExpClickScatterLMER}
\caption{More positive perception of Group Performance relative to prior expectations moderates the relationship between perceived Joint Action Success and Group Click}
...
%The main effects of group performance expectation violation(\betavec 0.01 \CIstart -0.27, 0.28 \CIfinish \SE 0.14, t(53) = 0.06, \pvalue 0.96) and experiment condition (\betavec 0.56 \CIstart -0.12, 1.25 \CIfinish \SE 0.35, t(53) = 1.6094197, \pvalue 0.11) were not significant predictors of social bonding. The result for the fixed effect of condition did appear to be trending in the predicted direction ($p = .11$).

Model residuals were normally distributed around zero (\resdist 0.99, \pvalue 0.92) and all \cooksD were less than X, see model assumptions in Appendix  ~\ref{fig:M1aAssumptions}).  These results suggested that the relationship between positive violation of group performance expectations and social bonding was significant, only in the high difficulty condition, and not overall across conditions.
