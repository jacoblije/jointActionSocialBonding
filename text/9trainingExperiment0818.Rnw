...
<<dataExclude, eval=TRUE, echo=FALSE, results='hide'>>=

twc <- subset(twc, successTotals > 4)
tlc <- subset(tlc, successTotals > 4)
...
<<arousalFactorPreConditionLMER, echo=F,eval=T>>=
  ...
  twc$condition <- factor(twc$condition, levels = c("low", "high"), ordered = F)
...
summary(conditionGroupPerf.main <- lmer(groupPerformance_3Norm ~ condition
                            + (1|sessionName)
                            , data = twc
                            , REML = F
                          )
...
summary(conditionGroupPerf.controls <- lmer(groupPerformance_3Norm ~ condition
...
<<groupPerfExpBondingConditionHist, echo=F,eval=T>>=
  ...
  hist(groupPerfExpBondingConditionResid, freq = FALSE, main = "Distribution of residuals", xlab = "Residuals")
...
<<groupPerfExpBondingConditionScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(groupPerfExpBondingCondition.controls, main = "Scatterplot of residuals")
  dev.copy(pdf, "TEM3Scatter.pdf")
  dev.off()
@
<<groupPerfExpBondingConditionQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(groupPerfExpBondingCondition.controls), main = "Norm QQ Plot of residuals")
...
#  library(lmerTest)
  ...
  summary(m3 <- lmer(groupBondingFactorPost ~  groupPerformance_3Norm + groupClickPostFactor + groupPerformance_3Norm*condition
                                      + arousalFactorPost
                                      + fullOutcomeAvg
                                      + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                      + tipiExtraverted
...
anova(perfBondingChange.lmer, perfBondingChangeCondition.lmer)
...
<<perfBondChangeAssumptionsScatter, echo=F,eval=T>>=
  #2.scatterPlot
  plot(perfBondingChangeCondition.lmer, main = "Scatterplot of residuals \n (standardised)")
  dev.copy(pdf, "TEM23Scatter.pdf")
  dev.off()
@
<<perfBondChangeQQNorm, echo=F,eval=T>>=
  #3.qqNorm
  qqnorm(residuals(perfBondingChangeCondition.lmer), main = "Norm QQ Plot of residuals")
  ...
  detach_package <- function(pkg, character.only = FALSE)
{
 ...
 #library(lmerTest)
  ...
  summary(m3Change <- lmer(groupBondingChangePrePost ~  groupClickChangePrePost*condition
                                          + groupPerfChangePrePostNorm*condition
                                          + indPerfChangePrePostNorm
                                          + fullOutcomeAvg
                                          + subjectiveCompetenceFactor + objectiveCompetenceFactor
                                          + tipiExtraverted
